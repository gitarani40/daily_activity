...
class DbHelper {
  ...
  void _createDb(Database db, int version) async {
    await db.execute("CREATE TABLE activity(id INTEGER PRIMARY KEY, name TEXT, durasi TEXT)");
    print("DB Created");
  }
  ...
  Future<int> insert(Activity activity) async {
    Database db = await this.database;
    int count = await db.insert('activity', activity.toMap());
    return count;
  }

  Future<int> update(Activity activity) async {
    Database db = await this.database;
    int count = await db.update('activity', activity.toMap(),
    where: 'id=?',
  ...
  Future<int> delete(Activity activity) async {
    Database db = await this.database;
    int count = await db.delete('activity', activity.toMap(),
    where: 'id=?',
    whereArgs: [activity.id]);
    return count;
  }
